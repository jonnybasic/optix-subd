...
void Shape::saveObjFile( const std::string& objFile ) const
{
    ...
    os << "# Vertices: " << verts.size() << "\n";
    os << "# Faces: " << nvertsPerFace.size() << "\n\n";
    ...
    for( size_t i = 0; i < nvertsPerFace.size(); ++i )
    {
        const int nVerts = nvertsPerFace[i];
        os << "f";
        
        for( int j = 0; j < nVerts; ++j )
        {
            const int vertIdx = faceverts[faceVertIndex + j] + 1; // OBJ indices are 1-based
            
            if( hasUV() )
            {
                const int uvIdx = faceuvs[faceUVIndex + j] + 1; // OBJ indices are 1-based
                os << " " << vertIdx << "/" << uvIdx;
            }
            else
            {
